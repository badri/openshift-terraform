...
data "template_file" "inventory" {
  ...
  vars {
    ...
    master_node_entry                  = "${format("%s openshift_ip=%s openshift_schedulable=true openshift_node_group_name='node-config-master'", digitalocean_droplet.master.ipv4_address, digitalocean_droplet.master.ipv4_address)}"

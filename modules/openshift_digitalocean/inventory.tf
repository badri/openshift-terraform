...
data "template_file" "inventory" {
  ...
  vars {
    ...
    master_node_entry                  = "${format("%s openshift_ip=%s openshift_schedulable=true openshift_node_group_name='node-config-all-in-one'", digitalocean_droplet.master.ipv4_address, digitalocean_droplet.master.ipv4_address)}"
    nodes                              = "${join("\n", formatlist("%s openshift_ip=%s openshift_schedulable=true openshift_node_group_name='node-config-compute'", digitalocean_droplet.nodes.*.ipv4_address, digitalocean_droplet.nodes.*.ipv4_address))}"
